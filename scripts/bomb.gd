...
const BOMB_TEXTURES : Dictionary = {
	ADJACENT: preload("res://assets/pixle_art/bombs/adjacent_bomb.png"),
	DIAGONAL: preload("res://assets/pixle_art/bombs/diagonal_bomb.png"),
	FULL: preload("res://assets/pixle_art/bombs/full_bomb.png")
}
...
func _process(_delta):
	...
	update_warning_cells()

func detect_player_collision():
	if raycast.is_colliding() and raycast.get_collider() is Player:
		Global.reset()

func update_warning_cells():
	cells_to_detonate = get_cells_to_detonate(tilemap.local_to_map(global_position))
...
func detonate(player_cell : Vector2i):
	tilemap.static_objects.remove_at(tilemap.static_objects.find(self))
	for cell in cells_to_detonate:
		if cell == player_cell:
			Global.reset()
...
func get_cells_to_detonate(cell : Vector2i) -> Array:
...
# This wonderful code was writen by u/nwb712 tysm
...
# Calls level_base.gd so arrays can properly be updated in that, latter calls the local detonate
func die():
